import { VideoProps } from "../types/VideoProps";

const videoDetails: VideoProps[] = [
    {
        thumbnailURL:
            "https://i.ytimg.com/vi/-eDkV9yMBF0/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&rs=AOn4CLADRpsZArA0ADgRARUHCRxzy2n5oA",
        creatorDisplayURL:
            "https://yt3.ggpht.com/MeY_fGNrjVLV0PVOBN7dRWzMBS0y41YGm55LOaJ02cXV82a7Np9pYxxhHFqdYdncEy1I2cYR=s68-c-k-c0x00ffffff-no-rj",
        videoDurationInSeconds: "456",
        videoTitle: "17 Placements, 7 Rejections : Super 30 Journey So Far",
        videoCreator: "Harkirat Singh",
        videoViews: "7.5K",
        videoUploaded: "2 hours",
    },
    {
        thumbnailURL:
            "https://i.ytimg.com/vi/GetaI7KhbzM/hqdefault.jpg?sqp=-oaymwEcCNACELwBSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&rs=AOn4CLBiRp9xaUqpwu1Yx-ugnYHEKdlfOw",
        creatorDisplayURL:
            "https://yt3.googleusercontent.com/n3xYuhy3Vc0L_Vi_MYBtiSfUOGLSR6VpJCEgZMzl7GUaxZIbsFSs-AUgFqndTzyu9Nb0s_xS4g=s160-c-k-c0x00ffffff-no-rj",
        videoDurationInSeconds: Number(12 * 60 + 53).toString(),
        videoTitle:
            "Mini Project: How to Program a GPU? | CUDA C/C++ Mini Project: How to Program a GPU? | CUDA C/C++ Mini Project: How to Program a GPU? | CUDA C/C++ Mini Project: How to Program a GPU? | CUDA C/C++ Mini Project: How to Program a GPU? | CUDA C/C++",
        videoCreator: "0Mean1Sigma",
        videoViews: "16K",
        videoUploaded: "3 days",
    },
    {
        thumbnailURL:
            "https://i.ytimg.com/vi/VkQsafoUPBk/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&rs=AOn4CLACBows6wkXtI_W6eOfE29oeYhpoA",
        creatorDisplayURL:
            "https://yt3.ggpht.com/9gmHBLpvKFkLdIfDoI7uKjtSHbWnuZ4xxmNT-_N14qc4OFqwj0Nc7-a-dj1ceonuNmXvHe_YLA=s68-c-k-c0x00ffffff-no-rj",
        videoDurationInSeconds: Number(11 * 60 + 37).toString(),
        videoTitle: "becoming good at tests is easy, actually",
        videoCreator: "easy, actually",
        videoViews: "129K",
        videoUploaded: "6 days",
    },
    {
        thumbnailURL:
            "https://i.ytimg.com/vi/WMlXBkjyoyI/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&rs=AOn4CLDKIfok_-CJaFcNkTKOMWX_Twq7Pw",
        creatorDisplayURL:
            "https://yt3.ggpht.com/5bkdmnZGqNlyMAzBNc6_H7KB7suViZWcwHPKWjDBAGVydTB-liueCFklrNV3HzZy1qXVtlaJ=s68-c-k-c0x00ffffff-no-rj",
        videoDurationInSeconds: Number(20 * 60 + 46).toString(),
        videoTitle: "waking up at 5am to see if it's worth it",
        videoCreator: "grace garagan",
        videoViews: "957K",
        videoUploaded: "8 months",
    },
    {
        thumbnailURL:
            "https://i.ytimg.com/vi/_cz0ijnYBJ4/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&rs=AOn4CLATStHytFISPv3vCzSgr_IJ4-LlAg",
        creatorDisplayURL:
            "https://yt3.ggpht.com/LI39AqyjtC958vuwXphANpwObWqRuWQ-xnMEeUH0kN4Ca5Oq5oTOiS7bZqZQtjtyZ3kq_7z7dRc=s68-c-k-c0x00ffffff-no-rj",
        videoDurationInSeconds: Number(3 * 60 + 24).toString(),
        videoTitle: "Your chest needs these exercises",
        videoCreator: "Yellow Dude | Gravgear",
        videoViews: "410K",
        videoUploaded: "1 month",
    },
    {
        thumbnailURL:
            "https://i.ytimg.com/vi/dwY7w0k3j2Y/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&rs=AOn4CLB5CRkU5ViYGO74W8H2Zy0t-WlpJg",
        creatorDisplayURL:
            "https://yt3.ggpht.com/ytc/AIdro_nirgFQ7Xu_KDkUZcFBbbUdF1oIbjq2QKU8QJasAamYyNQ=s68-c-k-c0x00ffffff-no-rj",
        videoDurationInSeconds: Number(1 * 3600 + 51 * 60).toString(),
        videoTitle: "Classical Music for When You're on a Deadline",
        videoCreator: "HALIDONMUSIC",
        videoViews: "9.6M",
        videoUploaded: "1 year",
    },
];

export default videoDetails;
